...
const utils = {
  ...
  filterIgnoreParameters(inType, name, ignoreParameters) {
    ...
    if (name.indexOf("[0]") > -1) {
      ...
      if (ignoreParameterAllKeys.length > 0) {
        var containtsKey = ignoreParameterAllKeys.filter((ignoreName) =>
          name.startsWith(ignoreName)
        );
      ...
      if (inType == "query") {
        ...
        // console.log(ignoreParameterAllKeys)
        return !ignoreParameterAllKeys.some(
          (key) =>
            new RegExp(`^(${key}$|${key}[.[])`).test(name) ||
            new RegExp(key, "g").test(name)
        );
      } else {
  ...
  appendBasePath(paths, basePath) {
    ...
    try {
      if (
        this.checkUndefined(basePath) &&
        this.strNotBlank(basePath) &&
        basePath != "/"
      ) {
  ...
  filterIncludeParameters(inType, name, includeParameters) {
    ...
    if (name.indexOf("[0]") > -1) {
      ...
      if (includeParameterAllKeys.length > 0) {
        var containtsKey = includeParameterAllKeys.filter((includeName) =>
          name.startsWith(includeName)
        );
        if (containtsKey.length > 0) {
          return true;
        } else {
  ...
  rootKeysPath(rootName, jsonInstance, includeKeys) {
    ...
    if (jsonInstance != null && jsonInstance != undefined) {
      for (var key in jsonInstance) {
        ...
        var parentKeyFlag = includeKeys.some((key) =>
          key.startsWith(tmpRootName)
        );
        if (!parentKeyFlag) {
          ...
          if (Array.isArray(tmpJson)) {
            ...
            keyArrs = keyArrs.concat(
              this.rootKeysPath(tmpArrName, tmpJson[0], includeKeys)
            );
            // keyArrs = keyArrs.concat(this.rootKeysPath(tmpRootName, tmpJson[0]));
          } else {
            if (isObject(tmpJson)) {
              keyArrs = keyArrs.concat(
                this.rootKeysPath(tmpRootName, tmpJson, includeKeys)
              );
            }
          }
        }
  ...
  checkParamArrsExists: function (arr, param) {
    ...
    return (arr || []).some(
      (row) => row.name + row.in == param.name + param.in
    );
    //  var flag = false;
